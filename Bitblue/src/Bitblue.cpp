...
Bitblue::Bitblue(char Left, char Right, char Up, char Down, char Select, char Start, char A, char B, char Y, char X){
	key[0] = Left;
	key[1] = Right;
	key[2] = Up;
	key[3] = Down;
...
void Bitblue::initialize(int baud_rate){
...
void Bitblue::write(byte data1, byte data2){
	#if ADD_SERIAL
...
bool Bitblue::iskeypadRead_complete(){ //Acknowledge if data is fullset and can also use as buffer has been read or not
...
void Bitblue::read_joystix(){ //read the data in serial and store in the rx_buffer
	#if ADD_SERIAL
...
bool Bitblue::isjoystixRead_complete(){ //Acknowledge if data is fullset and can also use as buffer has been read or not
	if(data_type <= 1){ //incomplete data in the buffer
		check_data_type(); //check contents in the rx_buffer
	}
...
bool Bitblue::check_first_buffer_content(){ //check if first data received in the keymapping list
...
void Bitblue::clear_buffer_content(){
	#if DEBUG
	Serial.write("CLR:");
...
char Bitblue::getkeypad(){
	char keypad_value = rx_buffer[0];
	clear_buffer_content();
	return keypad_value;
}

void Bitblue::getjoystix(byte *direction, byte *left_motor_pwm, byte *right_motor_pwm){
...
byte Bitblue::char_to_byte(byte ptr){
    byte converted=0;
...
bool Bitblue::isNonzero_content(byte ptr){
	for( ; (ptr % data_type) != 0; ptr++){
		if(rx_buffer[ptr] != '0'){
			return true;
		}
